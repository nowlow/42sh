...
char *cat_with_path(shell_t *shell, char *arg)
{
    ...
    with_slash = strmerge(my_getenv("HOME", shell->env), "/");
    with_home = strmerge(with_slash, arg);
    return with_home;
}
...
char *change_it(cmd_t *cmd, shell_t *shell)
{
    ...
    if (cmd->argc == 2) {
        if (my_strcmp(cmd->argv[1], "-")) {
            ...
            path = strmerge(path, getcwd(buffer, FILENAME_MAX));
        } else {
            chdir(my_getenv("OLDPWD", shell->env));
            path = strmerge(path, my_getenv("OLDPWD", shell->env));
        }
    } else {
        chdir(my_getenv("HOME", shell->env));
        path = strmerge(path, my_getenv("HOME", shell->env));
    }
    return path;
}
